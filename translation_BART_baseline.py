...
if __name__ == '__main__':
    ...
    if ARGS.gmodel == 'bart': # BART_{SPAN}
        ...
        print('\n== Our ==')
        if ARGS.use_token_importance_file:
            words_list, word_importances_list = [], []
            ...
            ### Pre-processing of Masking               
            labeled_train_data['source'] = labeled_train_data.apply(lambda row : gmodel_input_ids(row), axis=1)
            labeled_train_data['is_word_start'] = labeled_train_data.apply(lambda row : get_is_word_start(row), axis=1)
            labeled_train_data['word_to_bart_token_aligns'] = labeled_train_data.apply(lambda row : get_word_to_bart_token_aligns(row), axis=1)
        ...
        for target_cls in classes:    
            ...
            while k < diff:
                print('%s/%s-th sentence in class %s' % (k, diff, target_cls), end="\r")
...
#                         gen_set.add(generated_str)                
                ...
                for index, word, word_importance in sorted_tuples[:-1]:
                    if index / len(sorted_tuples) < 0.2:
                        continue
...
#                     gen_set.add(generated_str)
                    ...
                    #print(target_cls, pred, generated_str)
